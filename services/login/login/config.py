...
class ConfigurationException(Exception):
    """
    Raised when login service has a configuration error.
    """
...
def config_logging():
    ...
    ``logging.yaml`` for the application's logging configuration.

    This function overrides the ``login`` logger's default log level if another
    log level was provided using environment variables.
    """
    log_level = get_settings().log_level
    if log_level:
        logging.getLogger("login").setLevel(log_level.value)
